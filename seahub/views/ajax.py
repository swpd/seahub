...
def delete_dirents(request, repo_id):
    ...
    for dirent_name in dirents_names:
        try:
            seafile_api.del_file(repo_id, parent_dir, dirent_name, username)
            deleted_dirents.append(dirent_name)
        except SearpcError, e:
            logger.error(e)
            undeleted_dirents.append(dirent_name)

    return HttpResponse(json.dumps({'deleted_dirents': deleted_dirents,'undeleted_dirents': undeleted_dirents}),
                            content_type=content_type)
